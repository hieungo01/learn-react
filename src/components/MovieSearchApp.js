...
// 2dc9a6245210f21d037fa64df3ce7361
// https://api.themoviedb.org/3/movie/550?api_key=2dc9a6245210f21d037fa64df3ce7361
const MovieSearchApp = () => {
  const [movies, setMovies] = useState([]);
  const [search, setSearch] = useState("");
  const queryDebounce = useDebounce(search, 2000);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    async function fectchData() {
      setLoading(true);
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=2dc9a6245210f21d037fa64df3ce7361&query=${queryDebounce}`
      );
  ...
  return (
    <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
      <div className="h-[297px]">
        <LoadingSkeleton height="100%" borderRadius="16px"></LoadingSkeleton>
      </div>
...
const MovieItem = ({ data }) => {
  return (
    <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
      ...
      <div className="p-7 flex-1 flex flex-col">
        <h3 className="text-xl text-black font-semibold my-4">{data.title}</h3>
        <p className="text-[#999] text-sm mb-6 !leading-loose">
          {data.overview}
        </p>
        <div className="flex item-center gap-x-3 mt-auto">
